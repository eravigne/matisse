...
energie_dom_surf<-function(menage_echelle_2025){
  
  menage_ener_dom<-menage_echelle_2025
...
# A02
prix_classe_2025$prix_elec<- prix_classe$prix_elec * as.numeric(TC_prix %>% filter(CODADEME=="A02") %>% select(TC))

# A03
prix_classe_2025$prix_gaz<- prix_classe$prix_elec * as.numeric(TC_prix %>% filter(CODADEME=="A03") %>% select(TC))

# A04
prix_classe_2025[c("prix_fuel","prix_gpl","prix_bois","prix_chaleur")]<- prix_classe[c("prix_fuel","prix_gpl","prix_bois","prix_chaleur")]* as.numeric(TC_prix %>% filter(CODADEME=="A04") %>% select(TC))
...
menage_ener_dom$vol_Elec<-menage_ener_dom$Elec/menage_ener_dom$prix_Elec
menage_ener_dom$vol_Gaz<-menage_ener_dom$Gaz/menage_ener_dom$prix_Gaz
menage_ener_dom$vol_GPL<-menage_ener_dom$GPL/menage_ener_dom$prix_GPL
menage_ener_dom$vol_Fuel<-menage_ener_dom$Fuel/menage_ener_dom$prix_Fuel
menage_ener_dom$vol_Solides<-menage_ener_dom$Solides/menage_ener_dom$prix_Solides
menage_ener_dom$vol_Urbain<-menage_ener_dom$Urbain/menage_ener_dom$prix_Urbain
...
# 392 450 241
# C'est très peu comparé aux 431 TWh de 2010. Bien vérifier qu'avec les TC de Fred on a des chiffres plus cohérents. Conso énergétique des ménages qui diminue ... me parait peu plausible ...

menage_ener_dom$volpond_Elec<-menage_ener_dom$Elec/menage_ener_dom$prix_Elec*menage_ener_dom$pondmen
menage_ener_dom$volpond_Gaz<-menage_ener_dom$Gaz/menage_ener_dom$prix_Gaz*menage_ener_dom$pondmen
menage_ener_dom$volpond_GPL<-menage_ener_dom$GPL/menage_ener_dom$prix_GPL*menage_ener_dom$pondmen
menage_ener_dom$volpond_Fuel<-menage_ener_dom$Fuel/menage_ener_dom$prix_Fuel*menage_ener_dom$pondmen
menage_ener_dom$volpond_Solides<-menage_ener_dom$Solides/menage_ener_dom$prix_Solides*menage_ener_dom$pondmen
menage_ener_dom$volpond_Urbain<-menage_ener_dom$Urbain/menage_ener_dom$prix_Urbain*menage_ener_dom$pondmen
...
menage_ener_dom[paste("energie",sources,"surf",sep="_")]<- menage_ener_dom %>% select(starts_with("vol_")) %>% lapply(.,function(x) x/menage_ener_dom$surfhab_d)

mean(menage_ener_dom$energie_tot_surf)
# [1] 0.135696
# en 2010 
# [1] 0.1812944
...
dep_source_usage<<-dep_source_usage %>% mutate(ener_dom_surf=ener_dom_surf*1000) %>% select(ident_men,ener_dom_surf,ener_dom)
