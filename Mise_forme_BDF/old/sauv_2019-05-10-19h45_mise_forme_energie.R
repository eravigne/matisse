...
# Selection des ménages en commun dans appmen_intensities et menage_calibr_2010
appmen_intensites<-appmen_intensites[which(appmen_intensites$ident_men %in% intersect(menage_calibr_2010$ident_men,appmen_intensites$ident_men)),]
...
# menage_calibr_2010%>% filter(ident_men==i) %>% select(dep_sources)
...
# table(dep_source_usage$Elec-menage_calibr_2010$dep_Elec<10^(-4))
# View(cbind(menage_calibr_2010[which(dep_source_usage$Elec-menage_calibr_2010$dep_Elec<10^(-4)),c("ident_men","dep_Elec")],dep_source_usage[which(dep_source_usage$Elec-menage_calibr_2010$dep_Elec<10^(-4)),c("ident_men","Elec")]))
...
# Cohérence avec menage_calibr_2010 (dep_elec, dep_gaz, etc) => NOn (reventilation!)
table(dep_source_activite$Elec==menage_calibr_2010$dep_Elec)
table(dep_source_activite$Gaz==menage_calibr_2010$dep_Gaz)
table(dep_source_activite$GPL==menage_calibr_2010$dep_GPL)
table(dep_source_activite$Fuel==menage_calibr_2010$dep_Fuel)
table(dep_source_activite$Solides==menage_calibr_2010$dep_Solides)
# table(dep_source_activite$Solides-menage_calibr_2010$dep_bois-menage_calibr_2010$dep_charbon<10^(-12.8)) # les 217 erreurs <e-13
table(dep_source_activite$Urbain==menage_calibr_2010$dep_Urbain)
...
# (le chaffage pour le sommeil) qu'on multiplie par le pourcentage lié à une source d'énergie (gaz et sommeil) : on somme cette quantité pour toutes les activités. La seule exception est ElecSpé puisque ventiler des dépenses d'ElecSpé sur de l'Urbain ou du Fuel n'a pas de sens.


# la quantité d'ElecSpé est retirée de la consommation électrique des ménages
dep_ener$Elec<-dep_source_activite$Elec-dep_usage_activite$ElecSpe
dep_ener$Elec_ElecSpe<-dep_usage_activite$ElecSpe
usages_bis<-usages[1:5] #exclusion ElecSpe
...
for (usage in usages_bis){
  for (source in sources){
    ...
    for (act in activites){
      ...
      if(source=="Elec"){
        X<-(Table[source_act]-Table[paste("ElecSpe",act,sep="_")])*
...
dep_ener$Elec<-dep_ener$Elec+dep_usage_activite$ElecSpe
...
for (source in sources){
  ...
  print(table(dep_ener[source]-menage_forme_2010_rehab[dep_source]<10^(-5)))
}
...
for (source in sources){
  dep_source=paste("dep",source,sep="_")
  factor<-(1+(menage_calibr_2010[dep_source] - menage_forme_2010_rehab[dep_source])/menage_forme_2010_rehab[dep_source])
...
# puis que ener de dep_ener_reventil vaut bien menage_calibr_2010 dep_source
...
for (source in sources){
  ...
  dep_source=paste("dep",source,sep="_")
  # print(table(dep_ener_reventil[source]-menage_calibr_2010[dep_source]<10^(-6)))
}
...
# Cohérence avec menage_calibr_2010 (dep_elec, dep_gaz, etc) => OUI
table(dep_ener_reventil$Elec-menage_calibr_2010$dep_Elec<10^(-6))
table(dep_ener_reventil$Gaz-menage_calibr_2010$dep_Gaz<10^(-6))
table(dep_ener_reventil$GPL-menage_calibr_2010$dep_GPL<10^(-6))
table(dep_ener_reventil$Fuel==menage_calibr_2010$dep_Fuel)
table(dep_ener_reventil$Solides==menage_calibr_2010$dep_Solides)
table(dep_ener_reventil$Urbain==menage_calibr_2010$dep_Urbain)


# SAVE FILE ---------------------------------------------------------------

dep_ener_2010<-dep_ener_reventil %>% filter(ident_men %in% menage_calibr_2010$ident_men)
